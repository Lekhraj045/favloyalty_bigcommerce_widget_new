...
import AnnouncementsSliderArea, {
  type AnnouncementItem,
} from "@/components/AnnouncementsSlider";
...
type CustomerStatus = "loading" | "guest" | "not_in_program" | "loaded";

export default function HomePage({ config }: { config?: WidgetConfig }) {
  const [customerStatus, setCustomerStatus] =
    useState<CustomerStatus>("loading");
  const [customerData, setCustomerData] = useState<CustomerData | null>(null);
  const [referAndEarnEnabled, setReferAndEarnEnabled] = useState(false);
  const [referAndEarnPoints, setReferAndEarnPoints] = useState(0);
  ...
  // When parent sends "widget opened", refetch customer data so we show latest from DB on reopen
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    // Allow API call when we have storeHash + channelId (customerId can be empty; widget opens immediately, customer may arrive via postMessage later)
    ...
    console.log("[FavLoyalty Home] effect: config=", {
      hasApiUrl: !!apiUrl,
      apiUrl: apiUrl || "(missing)",
      hasJwt: !!jwt,
      jwtLength: jwt && typeof jwt === "string" ? jwt.length : 0,
      channelId,
      storeHash: storeHash || "(missing)",
    ...
    fetch(url, {
      ...
      .then((data) => {
        if (cancelled) return;
        console.log("[FavLoyalty Home] effect: response data=", data);
        if (data.referAndEarnEnabled !== undefined)
        ...
        if (data.success && data.inLoyaltyProgram && data.userName != null) {
          setCustomerData({
            ...
            equivalentValue: data.equivalentValue ?? 0,
            currency: data.currency ?? "USD",
            tierSystemEnabled: data.tierSystemEnabled === true,
            tierDisplay: data.tierDisplay,
            tierIndex: data.tierIndex,
            tierMultiplier:
              typeof data.tierMultiplier === "number"
                ? data.tierMultiplier
                : undefined,
          });
          setCustomerStatus("loaded");
          console.log("[FavLoyalty Home] effect: -> loaded", data.userName);
        } else {
          setCustomerData(null);
  ...
  // When signed out (no customerId), do not repopulate announcements so default state stays
  useEffect(() => {
    const apiUrl = config?.apiUrl?.replace(/\/$/, "");
    const storeHash = config?.storeHash;
    const channelId = config?.channelId;
  ...
  return (
    <>
      ...
      <div className="px-4 pb-4 space-y-4 -mt-5 relative z-10 h-[calc(100vh-68px)] overflow-y-auto custom-scroller">
        {/* Points Card (when in program) or Join CTA (guest / not in program) or Loading */}
        {customerStatus === "loading" && (
          <div className="border border-[#DEDEDE] bg-white rounded-xl p-4 animate-pulse">
            <div className="flex items-center gap-2 h-8 bg-gray-100 rounded w-32" />
            <div className="mt-2 h-4 bg-gray-100 rounded w-24" />
          </div>
        )}
        ...
        {customerStatus === "loaded" && customerData && (
          <div className="border border-[#DEDEDE] bg-white rounded-xl p-4">
            ...
            >
              ...
              <span className="text-sm">{customerData.pointsUnit}</span>
            </div>

            <div className="flex justify-between items-center mt-2">
              <p className="text-xs text-gray-500">
                Equals to {Number(customerData.equivalentValue ?? 0).toFixed(2)}{" "}
        ...
        <div className="border border-[#DEDEDE] bg-white rounded-xl p-3 space-y-2">
          ...
          <p className="text-xs text-gray-500">
            Get {referAndEarnPoints > 0 ? referAndEarnPoints : "100"} Bonus{" "}
            {customerData?.pointsUnit ?? pointsUnit}
          </p>
        ...
        {announcements.length > 0 && (
          <div className="rounded-xl overflow-hidden">
            <AnnouncementsSliderArea announcements={announcements} />
          </div>
        )}
      </div>
    </>
